  ...
  public function up () {
    $this->db->query (
      "CREATE TABLE `users` (
        ...
        `permission` int(11) NOT NULL DEFAULT '0' COMMENT '權限，黑名單會員 1，一般會員 3，小編 5，程式設計員 9',
    ...
    $this->db->query (
      "INSERT INTO `users` (`id`, `email`, `password`, `name`, `avatar`, `permission`, `login_count`, `logined_at`, `created_at`, `updated_at`)
        VALUES (1, 'root@mazu.ioa.tw', '" . md5 ('123456') . "', 'OA', '', '9', 0, '" . date ('Y-m-d H:i:s') . "', '" . date ('Y-m-d H:i:s') . "', '" . date ('Y-m-d H:i:s') . "');"
    );
  }
